...
# цикл ограничен количество замен. Так как замены парные, то замен в 2 раза меньше,
# чем количество элементов базового списка.
while i < len(arr) // 2:
    new_arr.append(arr_odd[i])  # сначала добавляем элемент из списка с четными
    new_arr.append(arr_even[i])  # далее добавляем элемент из списка с нечеетными
    i += 1
